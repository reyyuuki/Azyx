...
class _ReaderControlsState extends State<ReaderControls> {
  ...
  void _onProgressBarTap(double progress) {
    if (widget.totalImages.value > 0) {
      final targetIndex = (progress.round() - 1).clamp(
        0,
        widget.totalImages.value - 1,
      );
      if (widget.onPageSelected != null) {
        widget.onPageSelected!(targetIndex);
      } else {
        widget.controller.jumpToPage(targetIndex);
      }
    }
  }
  @override
                    ...
                    child: Obx(
                      () => M3Slider(
                        value: widget.totalImages.value > 0
                            ? (widget.currentPage.value.toDouble() + 1.0).clamp(
                                1.0,
                                widget.totalImages.value.toDouble(),
                              )
                            : 1.0,
                        max: widget.totalImages.value > 1
                            ? widget.totalImages.value.toDouble()
                            : 2.0,
                        divisions: widget.totalImages.value > 1
                            ? widget.totalImages.value - 1
                            : 1,
                        onChanged: (value) {
                          _onProgressBarTap(value);
                          widget.currentPage.value = value.round() - 1;
                        },
                        onDragStart: (value) {
                          _onProgressBarTap(value);
                          widget.currentPage.value = value.round() - 1;
