...
const baseUrl = 'https://wuxia.click';
...
Future<dynamic> scrapeNovelDetails(String url) async {
  ...
  if (response.statusCode == 200) {
    ...
    var titleElement = document
        .querySelector('.mantine-Text-root.mantine-Title-root.mantine-1ra9ysm');
    var title = titleElement?.text.trim() ?? '';
    var authorElement =
    ...
    var reviews = reviewsElement?.text.trim().trimLeft().split(' ').first ?? '';
...
Future<Map<String, dynamic>> scrapeNovelWords(String url) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (previousChapterLink != null && previousChapterLink.contains('novel')) {
      previousChapterLink = '';
    } else {
    ...
    var currentChapter = document
        .querySelector('h1.mantine-Text-root.mantine-Title-root.mantine-3i9n9a')
        ?.text;
    var wordElements =
