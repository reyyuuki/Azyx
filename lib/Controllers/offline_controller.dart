...
class OfflineController extends GetxController {
  ...
  void deleteCategory(Category cat) async {
    ...
    for (var mediaId in idsToCheck) {
      final allCats = await isar.categorys
          .filter()
          .isMangaEqualTo(cat.isManga)
          .findAll();
      ...
      if (isOrphaned) {
        ...
        final allItems = await isar.offlineItems
            .filter()
        ...
        for (var item in allItems) {
          if (item.mediaData?.id?.toString() == mediaId) {
            await isar.writeTxn(
              () async => await isar.offlineItems.delete(item.id),
            );
            log(
              "Deleted orphaned offline item after category delete: $mediaId",
            );
          }
        }
      }
        ...
        if (isOrphaned) {
          ...
          for (var item in allItems) {
            if (item.mediaData?.id?.toString() == mediaId) {
              ...
              log(
                "Deleted orphaned offline item completely from library: $mediaId",
              );
            }
          }
        }
