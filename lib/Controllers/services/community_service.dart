...
void navigateToAuthorProfile(CommunityMedia item) {
  ...
  if (serviceType == ServicesType.simkl) {
    ...
    launchUrlString('https://anilist.co/user/${item.anilistUserId}');
...
void navigateToReasonAuthorProfile(
  ...
  if (serviceType == ServicesType.simkl && user.simklId != null) {
    ...
    launchUrlString('https://anilist.co/user/${user.anilistId}');
...
class ReasonUserProfile {
  ...
  bool matchesUser(ReasonUserProfile? other) {
    ...
    if (anilistId != null && anilistId! > 0 && anilistId == other.anilistId)
      return true;
    if (malId != null && malId! > 0 && malId == other.malId) return true;
    if (simklId != null && simklId! > 0 && simklId == other.simklId)
      return true;
  ...
  String? get displayName => anilistUsername ?? malUsername ?? simklUsername;
  String? get displayAvatar => anilistAvatar ?? malAvatar ?? simklAvatar;
}
class ReasonEntry {
  final String? author;
  final String text;
...
class CommunityService extends GetxController {
  ...
  RxBool filterByListEnabled = true.obs;
  RxBool filterCompleted = true.obs;
  RxBool filterWatching = true.obs;
  RxBool filterDropped = true.obs;
  RxBool filterPlanning = false.obs;
  RxBool filterPaused = false.obs;
  RxBool filterRepeating = false.obs;
  @override
  void onInit() {
    super.onInit();
    filterByListEnabled.value = KvHelper.get<bool>(
      'community_filterByListEnabled',
      true,
    );
    filterCompleted.value = KvHelper.get<bool>(
      'community_filterCompleted',
      true,
    );
    filterWatching.value = KvHelper.get<bool>('community_filterWatching', true);
    filterDropped.value = KvHelper.get<bool>('community_filterDropped', true);
    filterPlanning.value = KvHelper.get<bool>(
      'community_filterPlanning',
      false,
    );
    filterPaused.value = KvHelper.get<bool>('community_filterPaused', false);
    filterRepeating.value = KvHelper.get<bool>(
      'community_filterRepeating',
      false,
    );
    communityEnabled.value = KvHelper.get<bool>(
      'showCommunityRecommendations',
      true,
    );
    hideNsfw.value = KvHelper.get<bool>('hideNsfwRecommendations', true);
  }
  ...
  CommunityMedia? _processSimklCommunityEntry(
    ...
    final media = Media(
      ...
      image: entry.poster ?? '',
      rating: entry.score ?? '?',
      mediaType: isMovie ? MediaType.anime : MediaType.manga,
  ...
  RxBool communityEnabled = true.obs;
  RxBool hideNsfw = true.obs;
  ...
  List<CommunityMedia> getFilteredCommunityShows() {
    ...
    try {
      ...
      final userList = simkl.userMangaList;
      final filteredIds = userList
          .where(
            (item) =>
                _activeFilteredStatuses.contains(item.status?.toUpperCase()),
          )
          .map((item) => item.id)
          .toSet();
  ...
  List<CommunityMedia> getFilteredCommunityMovies() {
    ...
    try {
      final serviceHandler = Get.find<ServiceHandler>();
      final simkl = serviceHandler.onlineService;
      final userList = simkl.userAnimeList;
      final filteredIds = userList
          .where(
            (item) =>
                _activeFilteredStatuses.contains(item.status?.toUpperCase()),
  ...
  List<CommunityMedia> getFilteredCommunityAnimes() {
    ...
    try {
      ...
      final userList = onlineService.userAnimeList;
      final filteredIds = userList
          .where(
            (item) =>
                _activeFilteredStatuses.contains(item.status?.toUpperCase()),
  ...
  List<CommunityMedia> getFilteredCommunityMangas() {
    ...
    try {
      final serviceHandler = Get.find<ServiceHandler>();
      final onlineService = serviceHandler.onlineService;
      final userList = onlineService.userMangaList;
      final filteredIds = userList
          .where(
            (item) =>
                _activeFilteredStatuses.contains(item.status?.toUpperCase()),
  ...
  CommunityMedia? _processEntry(
    ...
    final primaryId = serviceType == ServicesType.mal
        ? entry.malId
        : entry.anilistId;
    if (primaryId == null || primaryId == 0) return null;
    final media = Media(
  ...
  Future<void> fetchCommunityAnime() async {
    ...
    if (_cachedServiceType != serviceType) {
      if (communityAnimes.isNotEmpty) communityAnimes.clear();
      if (communityMangas.isNotEmpty) communityMangas.clear();
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Utils.log('Fetched ${communityAnimes.length} community anime');
      } else {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        communityMangas.value = entries
            ...
            .toList();
        _cachedServiceType = serviceType;
        Utils.log('Fetched ${communityMangas.length} community manga');
      } else {
  ...
  Future<void> fetchCommunityShows() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        final entries = data
            .map((e) => SimklCommunityEntry.fromJson(e))
            .toList();
        communityShows.value = entries
            .map((entry) => _processSimklCommunityEntry(entry, false))
            .whereType<CommunityMedia>()
            .toList();
        Utils.log('Fetched ${communityShows.length} community shows');
      } else {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        communityMovies.value = entries
            ...
            .whereType<CommunityMedia>()
            .toList();
        Utils.log('Fetched ${communityMovies.length} community movies');
      } else {
  ...
  Future<void> fetchAll() async {
    try {
      ...
      Utils.log('Error in community fetchAll: $e');
    }
  }
  Future<void> refreshData() async {
    if (communityAnimes.isNotEmpty) communityAnimes.clear();
    if (communityMangas.isNotEmpty) communityMangas.clear();
    if (communityShows.isNotEmpty) communityShows.clear();
    if (communityMovies.isNotEmpty) communityMovies.clear();
    await fetchAll();
  }
  ...
  static Future<bool> checkIsAdmin({
    ...
    try {
      ...
      Utils.log('checkIsAdmin error: $e');
    }
    return false;
  }
  static Future<VoteResult?> fetchVotes(
    ...
    try {
      ...
      if (malUserId != null) queryParams['mal_user_id'] = malUserId.toString();
      if (simklUserId != null)
        queryParams['simkl_user_id'] = simklUserId.toString();
      final url = Uri.parse(
        '$_botBaseUrl/api/votes/$mediaType/$mediaId',
  ...
  static Future<VoteResult?> castVote({
    ...
    try {
      ...
      Utils.log('castVote ${resp.statusCode}: ${resp.body}');
    } catch (e) {
      Utils.log('castVote error: $e');
    }
    return null;
  }
  static Future<Map<String, dynamic>?> checkIfExists({
    ...
    try {
      final url = Uri.parse(
        '$_botBaseUrl/api/check/$mediaType/$id',
      ...
      Utils.log('CommunityService.checkIfExists error: $e');
    }
    return null;
  }
  static Future<String?> submitRecommendation({
    ...
    try {
      ...
      Utils.log('CommunityService.submitRecommendation error: $e');
  ...
  static Future<String?> editReason({
    ...
    try {
      ...
      Utils.log('CommunityService.editReason error: $e');
      return 'Network error: $e';
    }
  }
  static Future<String?> deleteReason({
    ...
    try {
      ...
      final url = Uri.parse(
        '$_botBaseUrl/api/delete_reason/$mediaType/$mediaId',
      );
      ...
      Utils.log('CommunityService.deleteReason error: $e');
      return 'Network error: $e';
    }
  }
  static Future<(String? error, bool pending)> deleteReasonWithStatus({
    ...
    try {
      ...
      Utils.log('CommunityService.deleteReasonWithStatus error: $e');
  ...
  static Future<String?> deleteEntry({
    ...
    try {
      ...
      Utils.log('CommunityService.deleteEntry error: $e');
      return 'Network error: $e';
    }
  }
  static Future<(String? error, bool pending)> deleteEntryWithStatus({
    ...
    try {
      ...
      Utils.log('CommunityService.deleteEntryWithStatus error: $e');
      return ('Network error: $e', false);
    }
  }
...
class CommunityMedia {
  ...
  String get displayTitle => media.title != null && media.title!.isNotEmpty
      ? media.title!
      : (fallbackTitle ?? 'Unknown');
  String get displayDescription => reason ?? media.description ?? '';
  String? get author => usernameFor(media.servicesType ?? ServicesType.mal);
